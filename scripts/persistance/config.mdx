...
<ResponseField name="Config.Framework" type="string">
  The framework you are using. Currently supports `"qb"` or `"qbx"` or `"esx"` or `"standalone"`. For standalone, you need to add your own bridge to the script. More info at [Standalone Bridge](/scripts/persistance/standalone-bridge).
</ResponseField>

<ResponseField name="Config.KeysSystem" type="string">
  The vehicle keys script you are using. Currently supported: `"qb-vehiclekeys"`, `"qs-vehiclekeys"`, `"standalone"`. For standalone, you need to configure `bridge/keys/standalone`.
</ResponseField>

<ResponseField name="Config.Garage" type="string">
  The garage system you are using. Currently supported: `"qb-garages"`, `"esx_garage"`, `"jg-advancedgarages"`, `"vms_garagesv2"`, `"standalone"`. For standalone, you need to configure `bridge/garage/standalone`.
</ResponseField>
...
<ResponseField name="Config.Client.propsApplyTryCount" type="number" default="2">
  Number of attempts to re-apply vehicle properties (mods, colors) if a mismatch is detected after spawning. Default is 2. Do not change this value unless you know what you are doing.
</ResponseField>
...
<ResponseField name="Config.Server.timeToDeleteTemp" type="number" default="2">
  The age (in hours) at which temporary vehicles are automatically deleted. Set to `false` to disable deletion of temporary vehicles. Only applicable when `Config.Server.timeToDeleteActive` is `true`.
</ResponseField>

<ResponseField name="Config.Server.timeToDeletePerm" type="number" default="false">
  The age (in hours) at which permanent vehicles are automatically deleted. Set to `false` to disable deletion of permanent vehicles. Only applicable when `Config.Server.timeToDeleteActive` is `true`.
</ResponseField>
